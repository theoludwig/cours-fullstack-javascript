...
const MessageCard = forwardRef((props, ref) => {
  ...
  const handleSubmit = async () => {
    await api.put(`/messages/${props.message.id}`, {
  ...
  return (
    <>
      <div ref={ref} className='MessageCard'>
        <div className='row'>
          <span className='important'>{props.message.pseudo}</span> -{' '}
          {date.format(new Date(props.message.createdAt), 'DD/MM/YYYY à HH:mm')}
        </div>
        <div className='row'>
          {isEditing ? (
            <div className='text-center'>
              <form className='margin-top container'>
                ...
                <div className='margin-top MessageCard__options'>
                  <span onClick={handleSubmit} className='important'>
                    modifier
                  </span>{' '}
                  -{' '}
                  <span onClick={() => handleEdit(false)} className='important'>
                    annuler
                  </span>
                </div>
              </form>
          ...
          {props.pseudo === props.message.pseudo && !isEditing && (
            <div className='MessageCard__options'>
              <span onClick={deleteMessage} className='important'>
                supprimer
              </span>{' '}
              -{' '}
              <span onClick={handleEdit} className='important'>
                modifier
              </span>
            </div>
          )}
